...
function generateIITBombayFormat(data: any): string {
  ...
  if (data.experience && data.experience.length > 0) {
    ...
    data.experience.forEach((job: any) => {
      resume.push(`${job.title || "Position"} | ${job.company || "Company"} | ${job.duration || "XX/XXXX - XX/XXXX"}`);
  ...
  if (data.education && data.education.length > 0) {
    ...
    data.education.forEach((edu: any) => {
      resume.push(`${edu.degree || "Degree"} in ${edu.field || "Field"} | ${edu.institution || "University"} | ${edu.year || "XXXX"}`);
      if (edu.gpa) {
  ...
  if (data.skills && data.skills.length > 0) {
    ...
    resume.push(data.skills.map((skill: any) => (typeof skill === "string" ? skill : skill.name)).join(" • "));
    resume.push("");
  }
...
function generateOverleafLatexFormat(data: any): string {
  ...
  if (data.experience && data.experience.length > 0) {
    ...
    data.experience.forEach((job: any) => {
      resume.push(`\\textbf{${escapeLatex(job.title || "Position")}} $\\mid$ \\textit{${escapeLatex(job.company || "Company")}} \\hfill ${escapeLatex(job.duration || "XX/XXXX - XX/XXXX")} \\\\`);
      if (job.description || job.bullets) {
  ...
  if (data.education && data.education.length > 0) {
    ...
    data.education.forEach((edu: any) => {
      resume.push(`\\textbf{${escapeLatex(edu.degree || "Degree")}} in ${escapeLatex(edu.field || "Field")} $\\mid$ ${escapeLatex(edu.institution || "University")} \\hfill ${escapeLatex(edu.year || "XXXX")} \\\\`);
      if (edu.gpa) {
  ...
  if (data.skills && data.skills.length > 0) {
    ...
    const skillsText = data.skills.map((skill: any) => escapeLatex(typeof skill === "string" ? skill : skill.name)).join(" $\\cdot$ ");
  ...
  if (data.projects && data.projects.length > 0) {
    ...
    data.projects.forEach((project: any) => {
      resume.push(`\\textbf{${escapeLatex(project.title || "Project Name")}} \\hfill ${escapeLatex(project.duration || "XXXX")} \\\\`);
      if (project.bullets) {
        resume.push("\\begin{itemize}");
        project.bullets.forEach((bullet: string) => {
          resume.push(`  \\item ${escapeLatex(bullet)}`);
  ...
  if (data.certifications && data.certifications.length > 0) {
    ...
    resume.push("\\begin{itemize}");
    data.certifications.forEach((cert: any) => {
      resume.push(`  \\item ${escapeLatex(cert.title || "Certification")} - ${escapeLatex(cert.issuer || "Issuer")}`);
    });
    resume.push("\\end{itemize}");
    resume.push("");
  }
